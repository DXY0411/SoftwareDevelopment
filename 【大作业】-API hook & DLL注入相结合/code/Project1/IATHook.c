...
)
{
	...
	do 
	{
		pOriginThunk = (IMAGE_THUNK_DATA*)( (UCHAR*)pHookBlock->pImageBase + pImportDescriptor->OriginalFirstThunk );//指向导入名称表(INT)的RAV*   
		pRealThunk = (IMAGE_THUNK_DATA*)( (UCHAR*)pHookBlock->pImageBase + pImportDescriptor->FirstThunk );//指向导入地址表（IAT）的RAV*
		...
		{                           //被导入函数的地址
			...
			{                                            //被导入函数的序号    
				if ( (USHORT)pHookBlock->pszRoutineName == LOWORD( pOriginThunk->u1.Ordinal) )
				{
					if ( bHook )
					{
						pHookBlock->pOrigin = (void*)pRealThunk->u1.Function;//目标被替换函数的地址
			...
			{
				pImportByName = (IMAGE_IMPORT_BY_NAME*)( (char*)pHookBlock->pImageBase + pOriginThunk->u1.AddressOfData );//指向输入名称表 
...
LONG _IATHook_Internal( __in IATHOOK_BLOCK* pHookBlock , __in BOOLEAN bHook)
{
	...
	do 
	{
		...
		if ( 0 == pNTHeaders->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].VirtualAddress )  //导入表的偏移地址
		{
			break;
		}
		
		if ( 0 == pNTHeaders->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].Size )            //导入表的大小
		{
			break;
		}
		...
		{
			pszImportDllName = (char*)pHookBlock->pImageBase + pImportDescriptor->Name;//pImportDescriptor->Name是DLL名字符串的偏移值
			//用pImportDescriptor遍历找到目标DLL在导入表中所在的位置
